...
class LotsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create lot" do
    assert_difference('Lot.count') do
      post lots_url, params: { lot: { description: @lot.description, name: @lot.name, status: @lot.status, value: @lot.value } }
    end
  ...
  test "should update lot" do
    patch lot_url(@lot), params: { lot: { description: @lot.description, name: @lot.name, status: @lot.status, value: @lot.value } }
