class LandingPage::TheSpeakersController < ApplicationController
  before_action do
  ...
  def index
    @the_speakers =LandingPage::TheSpeaker.all
  end
  ...
  def new
    @the_speaker =LandingPage::TheSpeaker.new
  end
  ...
  def create
    @the_speaker =LandingPage::TheSpeaker.new(the_speaker_params)

    respond_to do |format|
      if @the_speaker.save
        format.html { redirect_to sections_edit_path, notice: 'Palestrante criado com sucesso.' }
  ...
  def update
    respond_to do |format|
      if @the_speaker.update(the_speaker_params)
        format.html { redirect_to sections_edit_path, notice: 'Palestrante atualizado com sucesso.' }
  ...
  def destroy
    @the_speaker.destroy
    respond_to do |format|
      format.html { redirect_to sections_edit_path, notice: 'Palestrante excluído com sucesso.' }
  ...
  private
    ...
    def set_the_speaker
      @the_speaker =LandingPage::TheSpeaker.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def the_speaker_params
      params.require(:the_speaker).permit(:title, :description, :image, :document)
    end
end
